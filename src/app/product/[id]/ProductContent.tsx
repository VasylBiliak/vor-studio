...
import { selectDefaultLanguage } from '@/store/slices/i18nSlice';
...
export default function ProductContent({ id }: ProductContentProps) {
  ...
  if (isLoading) {
    return (
      <div className='p-10 text-center uppercase tracking-widest text-xs'>
        {t('loading')}
      </div>
    );
  }
  ...
  const images: string[] =
    product.images.length > 0 ? product.images : [DEFAULT_IMG];
    ...
    <main>
      <div className='px-4 sm:px-6 md:px-10 pt-5 pb-12'>
        <div className='flex items-center gap-2 flex-wrap text-[10px] tracking-[1.5px] uppercase text-[var(--color-text-secondary)] mb-6'>
          <button
            onClick={() => router.push('/')}
            className='bg-transparent border-none cursor-pointer text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors'
          >
            {t('store')}
          </button>
          <span>/</span>
          ...
          <div className='lg:sticky lg:top-6'>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 bg-[var(--color-text-primary)] text-[var(--color-bg-primary)] text-xs tracking-[3px] uppercase font-bold cursor-pointer transition-colors hover:bg-[var(--color-accent-primary)]"
              >
                {t('buy_now') || 'Buy Now'}
              </button>

              {/* Add to Cart - Secondary Button with Icon */}
              <button
                onClick={() => {
                  if (product.sizes.length > 0 && !selectedSize) {
                    alert(t('select_size'));
                ...
                className="px-5 py-4 border border-[var(--color-text-primary)] bg-transparent text-[var(--color-text-primary)] cursor-pointer transition-colors hover:bg-[var(--color-text-primary)] hover:text-[var(--color-bg-primary)]"
      ...
      {relatedProducts.length > 0 && (
        <section id="products" className='px-4 sm:px-6 md:px-10 py-10 border-t border-[var(--color-border)]'>
